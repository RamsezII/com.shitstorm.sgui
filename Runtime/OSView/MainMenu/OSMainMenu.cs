        ...
        public void Toggle(in bool value)
        {
            switch (state_base)
            {
                ...
                case BaseStates.Enable:
                    if (!value)
                        animator.CrossFade((int)BaseStates.Enable_, 0, 0, 1 - animator.GetNormalizedTime01(0));
                    break;

                case BaseStates.Enable_:
                    if (value)
                        animator.CrossFade((int)BaseStates.Enable, 0, 0, 1 - animator.GetNormalizedTime01(0));
                    break;
